...
@implementation EStreamGiftBanner
...
- (void)setNextGiftMsg:(GiftChannelMessage *)nextGiftMsg
{
    if (_currentGiftMsg.fromuid.longLongValue == nextGiftMsg.fromuid.longLongValue &&
        ...
        CCLog(@"连发礼物乱序了，客户端丢弃,fromUid=%@ type=%lu tapCount=%d currentTapCount=%d",nextGiftMsg.fromuid,(unsigned long)nextGiftMsg.giftType,nextGiftMsg.tapCount,_currentGiftMsg.tapCount);
        return;

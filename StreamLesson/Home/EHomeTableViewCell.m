...
@property (nonatomic, strong) ChannelProfile        *cellModel;
...
@implementation EHomeTableViewCell
...
#pragma mark - 设置数据模型
- (void)setCellContent:(ChannelProfile *)model
{
    if (model) {
        self.cellModel = model;
        self.imgAvatar.image = nil;
        self.imgContent.image = nil;
        self.imgAvatar.image = [UIImage imageNamed:@"avatarSmall"];
        ...
        self.imgContent.image = [UIImage imageNamed:@"avatarBig"];
...
- (void)layoutSubviews
{
    ...
    //imgContent
    CGRect imgContentRect = CGRectMake(0, avatarRect.size.height + 2 * verticalMargin, self.contentView.bounds.size.width, SCREEN_WIDTH);
    //lbLiveStatus
    ...
    //lbFooter
    CGFloat originY = imgContentRect.origin.y + SCREEN_WIDTH;
